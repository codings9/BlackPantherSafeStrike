...
function StrikePrice { #START FUNCTION
    ...
    function 36MonthStrike_1 { #START FUNCTION

#        write-host "$CurrentPrice" -f green        
#        $CurrentPrice.GetType()

        $36MonCalc = ($ThreeYearPerformance / 1.5)
#        $18MonCalc = "{0:N2}" -f $18MonCalc
#        write-host "$18MonCalc" -f yellow

        $36MonShave = ($36MonCalc * 0.8)
#        $18MonShave = "{0:N2}" -f $18MonShave
#        write-host "$18MonShave" -f green

        $36MonShave = [math]::Round($36MonShave,2)
#        write-host "$18MonShave" -f cyan
#        $18MonShave.GetType()

        $36StrikeP = $CurrentPrice + $36MonShave
#        write-host "$18StrikeP" -f cyan -b blue

        $36MonthNumbers = $36MonCalc, $36MonShave, $36StrikeP
        $36MonthCalc = "{0:N2}" -f $36MonthNumbers
        $36MonthShave = "{1:N2}" -f $36MonthNumbers
        $36StrikePrice = "{2:N2}" -f $36MonthNumbers

        $36StrikePrice = [System.Convert]::ToDecimal($36StrikePrice)

#        write-host "$18MonthCalc" -f darkgreen -b cyan
#        write-host "$18MonthShave" -f darkred -b cyan
#        write-host "$18StrikePrice" -f blue -b cyan
        
        Set-Variable 36MonthCalc $36MonthCalc -Scope Script
        Set-Variable 36MonthShave $36MonthShave -Scope Script
        Set-Variable 36StrikePrice $36StrikePrice -Scope Script
...
#        write-host "$03StrikePrice" -f blue -b cyan
    ...
    #----------------------------------
      ...
      function 36MonthStrike_2 { #START FUNCTION

#        $CurrentPrice = 58.84
#        $TwoYearPerformance = 9.57

        $36MonCalc_2 = ($ThreeYearPerformance / 1.5)
        $36MonTTL_2 = $CurrentPrice + $36MonCalc_2

        $36MonShave_2 = ($36MonTTL_2 * 0.1)

        $36StrikeP_2 = $36MonTTL_2 - $36MonShave_2
        #$18StrikeP_2 = [math]::Round($18StrikeP_2,2)

        $36MonthNumbers_2 = $36MonCalc_2, $36MonTTL_2, $36MonShave_2, $36StrikeP_2
        $36MonthCalc_2 = "{0:N2}" -f $36MonthNumbers_2
        $36MonthTotal_2 = "{1:N2}" -f $36MonthNumbers_2
        $36MonthShave_2 = "{2:N2}" -f $36MonthNumbers_2
        $36StrikePrice_2 = "{3:N2}" -f $36MonthNumbers_2

        $36StrikePrice_2 = [System.Convert]::ToDecimal($36StrikePrice_2)
...
#    RequestTicker
 ...
 function FindThreeYrPerf { #START FUNCTION

        if ($matches -ne $null) { Clear-Variable matches }
        $ThreeYrPerf = $content2 -match '3-Year[\s\S]*?</tr>' | Out-Null  
        $ThreeYrPerf = [string]$matches.values -match '<td class="cell-period-change"> <div class="price  raising "> <span>(.*?)</span>'
#        $matches
        $ThreeYrPerf = $matches| foreach{$_.values}
        $ThreeYrPerf = $ThreeYrPerf[0]
#        write-host "$TwoYrPerf" -f yellow
        if ($ThreeYrPerf -like "+*"){
    
            $ThreeYrPerf = $ThreeYrPerf.replace("+","")
            $PosOrNeg = "POS"
        ...
        $ThreeYearPerformance = [System.Convert]::ToDecimal($ThreeYrPerf)
        Set-Variable ThreeYearPerformance $ThreeYearPerformance -Scope Script
...
#----------------------------------
    ...
    function WriteStrikePrice_1 { #START FUNCTION
       ...
       write-host -nonewline "Three-Year,24-month Exit : `$$36MonthCalc | Shaved Exit @ 80% : `$$36MonthShave | Strike : "
        write-host "`$$36StrikePrice" -f yellow -b black
        write-host -nonewline " Two-Year, 16-month Exit : `$$18MonthCalc | Shaved Exit @ 80% : `$$18MonthShave | Strike : "
        write-host "`$$18StrikePrice" -f yellow -b darkgreen
        write-host -nonewline " One-Year, 09-month Exit : `$$09MonthCalc | Shaved Exit @ 80% : `$$09MonthShave | Strike : "
        write-host "`$$09StrikePrice" -f yellow -b darkred
        write-host -nonewline " One-Year, 06-month Exit : `$$06MonthCalc | Shaved Exit @ 80% : `$$06MonthShave | Strike : "
        write-host "`$$06StrikePrice" -f cyan -b blue
        write-host -nonewline "Six-Month, 03-month Exit : `$$03MonthCalc | Shaved Exit @ 80% : `$$03MonthShave | Strike : "
    ...
    function WriteStrikePrice_2 { #START FUNCTION
        ...
        write-host -nonewline " Three-Year, 36-month Exit : `$$36MonthCalc | Shaved Exit @ 80% : `$$36MonthShave | Strike : "
        write-host "`$$36StrikePrice" -f yellow -b darkgreen
...
#    write-host "=============================================================" 
    ...
    write-host "-------------------------------------------------------------------------------------"
    write-host "                          Three-Year Performance  = `$$ThreeYearPerformance" -f cyan
    ...
    write-host "-------------------------------------------------------------------------------------"
    write-host ""
    write-host "                    *** BLACK PANTHER -SAFE- STRIKE PRICE ***                                " -f yellow
    write-host "-------------------------------------------------------------------------------------"
    WriteStrikePrice_1
    #write-host "-------------------------------------------------------------------------------------"
    #write-host ""
...
function FAQs { #START FUNCTION
    ...
    write-host "  not work for tickers not searchable on barchart.com.                                      " -f yellow -b black
    write-host "                                                                                            " -b black
...
function HeaderInfo { #START FUNCTION
    ...
    $filename = split-path $MyInvocation.PSCommandPath -Leaf
    $dateOfRelease="Released version: 10 OCT 2020"
